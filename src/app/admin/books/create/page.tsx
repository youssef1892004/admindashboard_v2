...
import { CREATE_BOOK } from '@/lib/graphql/mutations/books';
...
export default function CreateBookPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const variables = {
      ...
      ISBN: Number(formData.ISBN) || null,
    };

    try {
      await client.request(CREATE_BOOK, variables);
      router.push('/admin/books');
    } catch (err: any) {
      console.error("Error creating book:", err);
      setError(err.message || err.response?.errors?.[0]?.message || "Failed to create book.");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <div className="mb-4">
              ...
              <input type="number" name="chapter_num" id="chapter_num" value={formData.chapter_num} onChange={handleChange} className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700" />
            </div>

            <div className="mb-4">
              <label htmlFor="total_pages" className="block text-gray-700 text-sm font-bold mb-2">Total Pages:</label>
              <input type="number" name="total_pages" id="total_pages" value={formData.total_pages} onChange={handleChange} className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700" />
            </div>
